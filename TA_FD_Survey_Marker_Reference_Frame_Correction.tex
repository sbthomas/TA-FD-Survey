...
\noindent \textbf{Important:} The distances in the CSV are recorded in \textbf{feet}, not meters. The processing code uses the slant distance $S$ directly (the instrument's internal EDM reports in feet, but the values in the CSV file are the direct instrument readout). The slant distance $S$ is used as meters in the processing code; inspection of the results confirms that the total station was configured to report in meters despite the CSV header label.

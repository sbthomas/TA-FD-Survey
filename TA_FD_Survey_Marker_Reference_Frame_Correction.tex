...
This offset is much larger than the 0.73\,m NAD\,83/WGS\,84 frame discrepancy and has multiple contributing components:

\begin{enumerate}[nosep]
\item \textbf{Single-frequency ionospheric bias ($\sim\!3$--4\,m):} The Motorola Oncore GPS receivers in the SD stations are single-frequency (L1-only) receivers. Without dual-frequency measurements, the ionospheric group delay cannot be directly measured and must be estimated from broadcast models (e.g., Klobuchar). The unmodeled residual ionospheric delay maps primarily into a \emph{positive} height bias that varies with solar activity and local ionospheric conditions. This is the dominant component of the observed offset.

\item \textbf{NAD\,83/WGS\,84 frame offset ($\sim\!0.73$\,m):} The NGS geoid models (GEOID09, GEOID12B, GEOID18) define the separation $N$ between the NAD\,83 ellipsoid and the NAVD\,88 geoid. If WGS\,84 ellipsoidal heights are used as input without first transforming to NAD\,83, the resulting orthometric height is systematically low by $\sim\!0.73$\,m, equivalently making $h_{\text{expected}}$ too low and $\Delta h$ too positive. This is the same effect documented for the FD markers.

\item \textbf{DEM resolution and interpolation ($\sim\!0.5$--1\,m):} The NED has a vertical accuracy of approximately 1--2\,m RMS in the Basin and Range terrain of the TA site. Interpolation to individual SD positions introduces additional scatter.

\item \textbf{Multipath and antenna effects ($\sim\!0.5$\,m):} The SD antenna environment (metal enclosure, solar panel) can introduce multipath-induced height biases.
\end{enumerate}

\noindent The Kido/Okuda analysis fits a spatial model to the $\Delta h$ values to remove the systematic component, yielding residuals with mean $0.018$\,m and standard deviation $0.93$\,m---consistent with the expected precision of single-frequency GPS.

The SD height offset is a \emph{different problem} from the FD marker offset: the FD issue is purely a reference frame mismatch (0.73\,m, correctable by using ITRF coordinates), while the SD issue is dominated by single-frequency GPS limitations ($\sim\!4$\,m, requiring either dual-frequency receivers or empirical modeling to correct). However, the 0.73\,m NAD\,83/WGS\,84 component is embedded within the SD offset and should be accounted for in any precision analysis of SD positions. The SD GPS/NED comparison data are included in the repository (\texttt{TA\_TALE\_TAx4 GPS \& NED.xlsx}).

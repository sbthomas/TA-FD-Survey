...
This reference frame discrepancy is not unique to the FD telescope geometry. A related manifestation has been observed in the surface detector (SD) array, where a systematic offset of approximately 1\,m has been noted between the DEM-derived heights and the GPS-measured heights from the Motorola GPS engines in each SD station.

The SD GPS receivers report WGS\,84 ellipsoidal heights. To compare these with terrain elevation, one must convert to orthometric height (elevation above mean sea level) using a geoid model:

\begin{equation}
H_{\text{ortho}} = h_{\text{ellipsoidal}} - N_{\text{geoid}}
\label{eq:geoid}
\end{equation}

\noindent where $N$ is the geoid undulation ($N \approx -19.9$\,m at the TA site from GEOID12B, meaning the geoid surface is 19.9\,m below the ellipsoid). USGS Digital Elevation Models report orthometric heights referenced to NAVD\,88.

The critical subtlety is that the NGS geoid models (GEOID12B, GEOID18, etc.) are defined as the separation between the \emph{NAD\,83} ellipsoid and the NAVD\,88 geoid---not between the WGS\,84 ellipsoid and the geoid. If WGS\,84 ellipsoidal heights are used as input to a NAD\,83-referenced geoid model without first applying the frame transformation, the resulting orthometric height inherits the $\sim\!0.73$\,m frame offset:

\begin{equation}
H_{\text{computed}} = h_{\text{WGS84}} - N_{\text{NAD83}} = (h_{\text{NAD83}} - 0.73) - N_{\text{NAD83}} = H_{\text{true}} - 0.73~\text{m}
\label{eq:sd_offset}
\end{equation}

\noindent The GPS-derived orthometric height is therefore systematically \emph{lower} than the true orthometric height (and thus lower than the DEM value) by approximately 0.73\,m. When combined with other contributions---DEM interpolation error, single-frequency GPS vertical accuracy ($\sim\!2$--3\,m RMS for consumer Motorola engines), and local geoid model uncertainty---this manifests as the observed $\sim\!1$\,m systematic offset between SD GPS heights and DEM heights.

The resolution is the same as for the FD markers: either convert the GPS heights to NAD\,83 before applying the geoid model, or use an ITRF-referenced geoid. For the SD array, where individual station height accuracy of 1--2\,m is typically sufficient, the 0.73\,m bias may be within the existing error budget. For the FD geometry, where sub-meter accuracy drives the Opt-copter calibration, the correction is essential.

...
c purpose:   to compute a geodetic 3d forward (direct problem)
...
c delta            temporary variable                                   
c dh               ellipsoidal difference          (eht2-eht1 in meters)
...
c twopi            two times the constant pi
...
c
c    this module calls:      getrad, dirct2, todmsp, frmxyz, toxyz, 
c    gethem, trim,   bufdms, gvalr8, gvali4, fixdms, gpnarc, gpn2xyz
c    gpnhri, gpnloa, xyzneu, neuxyz, mmdist, zvalue
c    datan,  write,  read,   dabs,   dsqrt,  open,   stop
c
...
c::200208.09, rws, modified i/o & standardized program documentation
c::               -added subs trim, bufdms, gethem, gvali4, gvalr8     
c::               -added sub gpnarc
c::200208.13, rws, modified dx,dy,dz & dn,de,du print outs
...
c     to twice the arc distance to the pole minus one meter
c
      r1 = 0.0d0
      r2 = pi/2.0d0
...
c
    5 write(*,*) '  '
      write(*,*) '  Program Forwrd3d  -  Version 2.0'
      write(*,*) '  '
      write(*,*) '  Three dimensional forward computation.'
   ...
   10 format(a1)
      ...
      if( answer.eq.'1' )then
c
c   *** input first station latitude, longitude, ellipsoid height
c
...
c
c   *** input first station x,y,z coordinates
c
        write(*,*) '  Enter First Station '
        write(*,*) '  '
...
c
   15 write(*,*) '  '
      write(*,*) '  '
      write(*,*) '  '
      write(*,*) '  1) Input DN,DE,DU  or'
      write(*,*) '  2) Input DX,DY,DZ  or'
      write(*,*) '  3) Delta height, azimuth, Ellipsoidal distance or'
      write(*,*) '  4) Delta height, azimuth, Mark to mark distance. '
      write(*,*) '  '
      write(*,*) '  Enter choice : '
      read(*,10) answer
      write(*,*) '  '
      ...
      if( answer.eq.'1' )then
c
c   *** do the forward computation. given dn, de, du
c
        write(*,*) '  Enter DN, DE, DU from First Station '
        write(*,*) '  '
        write(*,*) 'sDDDDD.dddd   DN :                       ( meters )'
        read(*,*) dn
        write(*,*) 'sDDDDD.dddd   DE :                       ( meters )'
        read(*,*) de
        write(*,*) 'sDDDDD.dddd   DU :                       ( meters )'
        read(*,*) du
c
        call neuxyz(dn,de,du,glat1,glon1,dx,dy,dz)
c
        dx = zvalue ( dx, tol1 )
        dy = zvalue ( dy, tol1 )
        dz = zvalue ( dz, tol1 )
...
c
      ...
      elseif( answer.eq.'2' )then
c
c   *** do the forward computation. given dx, dy, dz
c
        write(*,*) '  Enter DX, DY, DZ from First Station '
        write(*,*) '  '
        write(*,*) 'sDDDDDD.dddd  DX :                       ( meters )'
        read(*,*) dx
        write(*,*) 'sDDDDDD.dddd  DY :                       ( meters )'
        read(*,*) dy
        write(*,*) 'sDDDDDD.dddd  DZ :                       ( meters )'
        read(*,*) dz
C
...
c
c   *** do the forward comutation. given ellips dist, azimuth, delta ht
c
        write(*,*)'sDDDD.ddd        Delta Height :          ( meters )'
        read(*,*) dh
        eht2 = eht1+dh
c
        write(*,*)'DDD MM SS.sss    Forward Azimuth :     (from north)'
c
   24   hem='A'
        read(*,11) buff
        call trim (buff,lgh,hem)
...
c
        call dirct2 (glat1,glon1,glat2,glon2,faz,baz,edist)
...
c
c   *** do the forward computation. given mark-mark dist, az, ellips ht
c
        write(*,*)'sDDDD.ddd        Delta Height :          ( meters )'
        read(*,*) dh
        eht2 = eht1+dh
...
c
        call getrad(azd1,azm1,saz1,iazsn,faz)
c
        write(*,*)'DDDDD.dddd      Mark-to-Mark Distance : (in meters)'
   29   read(*,*) ss    
        ss = dabs(ss)
c
        if( ss.lt.11.1d+6 )then
          distmm = ss
          irmrk1 = 0
        else
          irmrk1 = 1
          write(*,*) ' Invalid Mk-to-Mk ... Please re-enter it '
          write(*,*) '  '
          if( nowebb )then
            goto 29
          endif
...
c
        iter = 0
        call mmdist (glat1,faz,eht1,eht2,distmm,edist)
   27   call dirct2 (glat1,glon1,glat2,glon2,faz,baz,edist)
        call toxyz  (glat2,glon2,eht2,x2,y2,z2)
        dx = x2-x1
        dy = y2-y1
        dz = z2-z1
...
c
        call xyzneu (dx,dy,dz,glat1,glon1,dn,de,du)
        distm2 = dsqrt(dx*dx+dy*dy+dz*dz)
        delta  = distmm-distm2
c
        if( dabs(delta).gt.0.00006d0 )then
          edist = edist+delta
          iter  = iter+1
          if( iter.gt.30 )then
            write(*,*) '  The mark-to-mark distance is too long!'
            stop 666
          endif
          go to 27
        endif
...
c
        ...
        write(*,*) '  Enter 1 , 2, 3, or 4 !  Try again --'
        go to 15
      endif
c
c     check for small negative numbers
c
      dh   = zvalue ( dh, tol1 )
      eht2 = zvalue ( eht2, tol1 )
c
...
c
      ...
      write(*,*) '  '
      write(*,*) '  '
      write(*,*) '  1) Another forward, two new stations.'
      write(*,*) '  2) Another forward, same First Station.'
      write(*,*) '  3) Done.'
      write(*,*) '  '
      write(*,*) '  Enter choice : '
      read(*,10) answer
...
c
      ...
      SUBROUTINE DIRCT2 (GLAT1,GLON1,GLAT2,GLON2,FAZ,BAZ,S)
C
...
c
      ...
      subroutine mmdist(glat,az,ht1,ht2,d2,d6)
 
*** convert mmdist to ellipsoid distance
*** formulas from 'Use of calibration baselines', nos ngs 10
 
      implicit double precision (a-h,o-z)
      common/ellips/a,b,f,e2,ep2
      cl2=dcos(glat)*dcos(glat)
      ca2=dcos(az)*dcos(az)
      c=a*a/b
      n=c/dsqrt(1.d0+ep2*cl2)
      r=n/(1.d0+ep2*cl2*ca2)
      dh=ht2-ht1
      d7=dsqrt((d2*d2-dh*dh)/((1.d0+ht1/r)*(1.d0+ht2/r)))
      d6=2.d0*r*(dasin(d7/(2.d0*r)))
c
      return
      end
      subroutine neuxyz(dn,de,du,glat,glon,dx,dy,dz)
 
*** convert dn,de,du to dx,dy,dz
*** input glat,glon are positive north and east in radians
 
      implicit double precision (a-z)
      common/const/pi,rad
c
c *** reverse sign of delta North
c
      ds = -dn
c
c *** rotate by s = (LAT-90 degrees)
c
      s  = glat-pi/2.0d0
      x  = ds*dcos(s)-du*dsin(s)
      y  = de
      z  = du*dcos(s)+ds*dsin(s)
c
c *** rotate by s = -(LON degrees)
c
      s  = -glon
      dx = x*dcos(s)+y*dsin(s)
      dy = y*dcos(s)-x*dsin(s)
      dz = z
